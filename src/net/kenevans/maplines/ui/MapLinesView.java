...
public class MapLinesView extends ViewPart implements IPreferenceConstants
{
    ...
    public boolean linesFromGpx() {
        ...
        for(String name : files) {
            ...
            try {
                ...
                if(!res) {
                    SWTUtils.errMsg("Error reading GpxFile: " + fileName
                        + SWTUtils.LS + "Aborting");
                    return false;
                }
            } catch(Exception ex) {
    ...
    public void saveLinesImage(boolean withImage) {
        ...
        if(selectedPath != null) {
            ...
            lines.saveLinesImage(display, fileName, width, height,
                withImage ? viewer.getImage() : null);
        }
    }
    ...
    public void loadImage(String fileName) {
        ...
        try {
            ...
            // System.out.println(String.format(
            // " After Dispose Old Image: Free Memory: %.2f / %.2f (Max %.2f)
            // MB",
            // free / 1024. / 1024., total / 1024. / 1024., max / 1024. /
            // 1024.));
    ...
    protected void createHandlers() {
        ...
        handler = new AbstractHandler() {
            public Object execute(ExecutionEvent event)
                throws ExecutionException {
                keyPressed("Ctrl+Home");
                return null;
            }
        };
        id = "net.kenevans.maplines.ctrlhome";
        handlerService.activateHandler(id, handler);
